...
def single_profile_plot(scenario, commodity, region, year, CorRorA, fD, fP, fc, f, c, demcolor):
    ...
    for col in f:
        try:
            ps.append(ax.fill_between(x, temp, temp+f.loc[fD.index, col].values/1e3, label=col, facecolor=c[col]))
        except KeyError:
            print('No defined colour for %s'%col)
            ps.append(ax.fill_between(x, temp, temp+f.loc[fD.index, col].values/1e3, label=col))
